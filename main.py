...
def write_csv(all_details):
    ...
    with open(f"./scraped_user_data/{username}.csv", "w") as f:
        writer = csv.writer(f)
        writer.writerow(["userId", "username", "date_joined", "image_url", "cover_image_url", "about", "gabss", "followers", "following"])
        writer.writerow([all_details[0]["userId"], all_details[0]["username"], all_details[0]["date_joined"], all_details[0]["image_url"], all_details[0]["cover_image_url"], all_details[0]["about"], all_details[0]["gabss"], all_details[0]["followers"], all_details[0]["following"]])
        writer.writerow(["post_id", "favourites_count", "quotes_count", "reblogs_count", "replies_count"])
        for i in all_details[1]:
            writer.writerow([i["post_id"], i["favourites_count"], i["quotes_count"], i["reblogs_count"], i["replies_count"]])
